...
class Command:
    def __init__(self, players, kick, server_id):
...
# --- Settings --- #
    ...
    def color_list(self):
        return f"Entrez le code hexadécimal `+couleur 0xRRVVBB`, ou le nom de la couleur souhaitée.\n**Liste des couleurs disponibles**\n - " + "\n - ".join(data_color())
    ...
    def player_new(self, message):
        ...
        if species:
            ...
            return "*Erreur : syntaxe invalide : `+nouveau < nom_de_l'espèce >`.*"
    ...
    def modify_note(self, message):
        ...
        if not args or len(args) != 2:
            return "*Erreur : syntaxe invalide : `+note < contenu > , < + | - >`.*"

        if args[1] == "+":
            user.add_note(args[0])
            return f"{user.name} a ajouté la note :\n> {args[0]}"
        elif args[1] == "-":
            return user.del_note(args[0])
...
# --- Game --- #
    ...
    def roll_dice(self, message):
        ...
        return f"{user.name} lance {nb} dé{('', 's')[nb > 1]} à {faces} faces. Résultat obtenu : {roll_die(faces, nb)} / {faces * nb}."
    ...
    def roll_capacity(self, message):
        ...
        result = f"{user.name} a {('fait un échec critique sur', 'raté', 'réussi de justesse', 'réussi')[comment]} son lancer "
    ...
    def show_articles(self, message):
        ...
        if item_name:
            ...
            if item_name in data_shop()[shop_name]:
                return (item_name, object_stat(item_name)[0]), user.stat[8], 1
            else:
                return "*Erreur : cet objet n'est pas vendu ici. Consultez la liste des objets disponibles via : `+article`.*", -1, 2
    ...
    def buy(self, message):
        ...
        user.stat[7] += price
        return f"{user.name} a acheté {item_name} pour {abs(price)} Drachmes."
